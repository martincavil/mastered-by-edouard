...
export function ListenClient({ artists }: { artists: Artist[] }) {
  const t = useTranslations();
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  // Tous les artistes (ordonnés par `order`) pour la liste des noms
  const allArtists = artists;

  // Artistes mis en avant en images (cochés `featured` dans Sanity, avec une photo)
  const gridArtists: Artist[] = useMemo(
    () =>
      artists.filter(
        (artist) =>
          artist.featured && artist.picture && artist.picture.trim() !== "",
      ),
    [artists],
  );

  // Générer une grille en forme de losange (3-4-5-4-3)
